...
module Phreak
  class Subparser
    ...
    def bind(word : String | Nil = nil, long_flag : String | Nil = nil,
             short_flag : Char | Nil = nil, description : String | Nil = nil,
             &block : Subparser, String -> Nil) : Nil
      @bindings.push Binding.new(word, long_flag, short_flag, description, block)
    end
    ...
    def fuzzy_bind(word : String | Nil = nil, long_flag : String | Nil = nil,
                   description : String | Nil = nil, &block : Subparser, String -> Nil) : Nil
      @fuzzy_bindings.push Binding.new(word, long_flag, nil, description, block)
    end
    ...
    def grab(&block : Subparser, String -> Nil) : Nil
      @wildcard = Binding.new(nil, nil, nil, nil, block)
    end
    ...
    # Prints an indented block of commands with their matching description to
    # the given IO.
    protected def print_bindings(io : IO, bindings : Array(Binding))
      bindings.each_index do |index|
        binding = bindings[index]
        line = help_indent

        # This is just to make the code a little less clunky below.
        word = binding.word
        short = binding.short_flag
        long = binding.long_flag
        desc = binding.description || "No description provided."
        ...
        # This block ensures there is at least **some** padding between
        # the command and the description, even if the command portion
        # is longer than the normal width.
        if line.size >= command_section_width
          line += fallback_description_padding
        else
          line = line.ljust(command_section_width)
        end
